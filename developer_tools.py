"""
GitHub Developers Suite - Utility Functions (OPTIMIZED)
A collection of utility functions for GitHub developers with performance improvements
"""
...
def assess_skills(skills_dict):
    ...
    Improvements:
    - Single pass through dictionary items instead of multiple passes
    ...
    # OPTIMIZED: Single pass through items instead of multiple passes
    total_score = 0
    weak_areas = []
    strong_areas = []
...
_LEVELS = ["Junior", "Mid-Level", "Senior", "Staff", "Principal"]


def create_development_plan(current_level, target_level, timeline_months):
    """
    Create a development plan to progress from current to target level - OPTIMIZED
    ...
    for i, lvl in enumerate(_LEVELS):
        if lvl in current_level and current_idx == -1:
            current_idx = i
        if lvl in target_level and target_idx == -1:
            target_idx = i
    ...
    target_level_key = _LEVELS[target_idx]
    milestones = _LEVEL_MILESTONES.get(target_level_key, [])
...
def calculate_pr_quality_score(pr_data):
    """
    Calculate pull request quality score - OPTIMIZED
    
    Improvements:
    - Early extraction of values to avoid repeated dict.get() calls
...
if __name__ == "__main__":
    # Example usage
    print("GitHub Developers Suite - Utility Examples (Optimized)\n")
